...
\textbf{Architecture Overview.} Our system follows a three-stage pipeline: (1) \textit{Sketch Input}, where users create or upload 2D sketches, (2) \textit{AI Generation}, which produces material maps from sketches, and (3) \textit{Classical Rendering}, which visualizes the materials in 3D. 
...
\textbf{Classical Rendering Pipeline.} The rendering module implements a classical OpenGL 3.3 rasterization pipeline:
...
\textbf{Classical Graphics Components.}
...
\textbf{System Integration.} The system integrates AI and classical components through a two-mode interface:
\begin{itemize}
...
\textbf{Visual Quality Assessment.} We evaluate the visual quality of generated materials through qualitative analysis. Figure~\ref{fig:results} shows example results from various sketch inputs. The system successfully generates detailed textures that preserve the overall structure of input sketches while adding realistic surface details.
...
The Phong lighting model provides realistic illumination, with specular highlights enhanced by roughness map modulation. Displacement mapping creates visible geometric relief, while normal mapping adds fine-scale surface variation. The inclusion of roughness maps enables more physically accurate material appearance, with rougher surfaces exhibiting broader, dimmer specular highlights. The combination produces materials that appear three-dimensional and realistic.

\textbf{Performance Metrics.} Performance measurements were conducted on multiple hardware configurations:
...
\textbf{Integration Effectiveness.} The integration between AI and classical components demonstrates several advantages:
\begin{itemize}
...
\textbf{Findings.} Our implementation demonstrates that modern AI models can be effectively integrated with classical rendering pipelines. The combination of ControlNet-based Stable Diffusion for texture generation and learned depth estimation provides a complete PBR material creation workflow. The classical rendering pipeline ensures real-time performance and realistic lighting, while AI components handle the complex task of content generation.
...
\textbf{Limitations.} Several limitations are present in the current implementation:
...
The current implementation blocks the UI during AI generation. Asynchronous processing would improve user experience.
