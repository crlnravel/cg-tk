...
class Timer:
    ...
    def log(name, elapsed_time):
        """Log a timing result without context manager"""
...
class AIPipeline:
    ...
    def generate(self, prompt, init_image_path, project_name, progress_callback=None):
        ...
        with Timer("Albedo Generation (ControlNet Diffusion)"):
            albedo = self.pipe(
                pos_prompt,
                image=sketch,
        ...
        with Timer("Normal Map Generation (Sobel Edge Detection)"):
...
def ai_loader_thread():
    ...
    try:
        ...
        with Timer("AI Pipeline Initialization (Loading ControlNet + Stable Diffusion + Depth Estimator)"):
